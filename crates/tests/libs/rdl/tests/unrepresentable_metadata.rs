...
#[test]
fn writer_rejects_mixed_interface_method_implementations() {
    let mut file = metadata::writer::File::new("test");
    file.TypeDef(
        "Test",
        "IValue",
    ...
    for (name, flags) in [
        ("Runtime", metadata::MethodImplAttributes::Runtime),
        ("Abstract", metadata::MethodImplAttributes::default()),
    ] {
        file.MethodDef(
            name,
            &metadata::Signature {
    ...
    let error = writer_error("mixed_interface_impl", file);
    assert_eq!(
        error.message,
        "interface `IValue` mixes runtime and non-runtime methods"
    );
}
...
#[test]
fn writer_preserves_property_and_event_attributes() {
    let mut file = metadata::writer::File::new("test");
    ...
    let getter = file.MethodDef(
        ...
        metadata::MethodAttributes::Public | metadata::MethodAttributes::SpecialName,
        Default::default(),
    );
    ...
    let output = out_path("property_attribute");
    windows_rdl::writer()
        .input_bytes(&file.into_stream())
        .output(&output)
        .write()
    ...
    let handler = metadata::Type::class_named("Test", "Handler");
    let token = metadata::Type::value_named("Windows.Foundation", "EventRegistrationToken");
    let event = file.Event("Changed", &handler);
    file.EventMap(interface, event);
    let add = file.MethodDef(
        "add_Changed",
        &metadata::Signature {
            flags: metadata::MethodCallAttributes::HASTHIS,
            return_type: token.clone(),
            types: vec![handler],
        },
        metadata::MethodAttributes::Public | metadata::MethodAttributes::SpecialName,
        Default::default(),
    );
    file.Param("handler", 1, metadata::ParamAttributes::In);
    ...
    file.Param("token", 1, metadata::ParamAttributes::In);
    file.MethodSemantics(0x0008, add, metadata::writer::HasSemantics::Event(event));
    file.MethodSemantics(0x0010, remove, metadata::writer::HasSemantics::Event(event));
    let attribute = marker_attribute(&mut file);
    file.Attribute(metadata::writer::HasAttribute::Event(event), attribute, &[]);

    let output = out_path("event_attribute");
    windows_rdl::writer()
        .input_bytes(&file.into_stream())
        .output(&output)
        .write()
...
#[test]
fn writer_rejects_method_impls() {
    let mut file = metadata::writer::File::new("test");
    let ty = file.TypeDef(
        "Test",
        "Class",
    ...
    let parent = metadata::writer::MemberRefParent::TypeRef(file.TypeRef("Test", "IInterface"));
    let declaration = file.MemberRef("Method", &signature, parent);
    file.MethodImpl(
        ty,
        metadata::writer::MethodDefOrRef::MethodDef(method),
        metadata::writer::MethodDefOrRef::MemberRef(declaration),
    );

    let error = writer_error("method_impl", file);
    assert_eq!(
        error.message,
        "type `Test.Class` has MethodImpl mapping `Test.Class.Method` -> \
         `Test.IInterface.Method` with no RDL projection"
    );
}
