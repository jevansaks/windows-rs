...
#[test]
fn unused_and_shadowed_imports_are_warnings() {
...
"#,
    ...
    assert_eq!(
        ...
        windows_rdl::Severity::Warning
    );
    assert_eq!(report.diagnostics()[0].code.as_deref(), Some("RDL0006"));
    assert!(report.diagnostics()[0].message.contains("Other.Unused"));
    assert_eq!(report.diagnostics()[1].code.as_deref(), Some("RDL0007"));
    assert!(report.diagnostics()[1].message.contains("Other.Value"));
}

#[test]
fn import_warning_suppressions_are_scoped_to_each_use() {
    let report = windows_rdl::reader()
        .input_text_named(
            "imports.rdl",
            r#"
...
#[test]
fn used_named_and_glob_imports_do_not_warn() {
    let report = windows_rdl::reader()
        .input_text_named(
            "imports.rdl",
            r#"
...
#[test]
fn used_attribute_import_does_not_warn() {
    let report = windows_rdl::reader()
        .input_text_named(
            "imports.rdl",
            r#"
