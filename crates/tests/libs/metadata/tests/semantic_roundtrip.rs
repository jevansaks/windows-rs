...
fn input(path: &std::path::Path) {
    ...
    let ordinary_signature = Signature {
        flags: MethodCallAttributes::HASTHIS,
        return_type: Type::Void,
        types: vec![],
    };
    let ordinary = file.MethodDef(
        "Ordinary",
        &ordinary_signature,
        MethodAttributes::Public,
        MethodImplAttributes::Runtime,
    ...
    let declaration_parent =
        writer::MemberRefParent::TypeRef(file.TypeRef("Test", "IRuntimeClass"));
    let declaration = file.MemberRef("Ordinary", &ordinary_signature, declaration_parent);
    file.MethodImpl(
        ty,
        writer::MethodDefOrRef::MethodDef(ordinary),
        writer::MethodDefOrRef::MemberRef(declaration),
    );
...
fn verify(path: &std::path::Path, namespace: &str) {
    ...
    let method_impl = ty.method_impls().next().unwrap();
    assert!(matches!(
        method_impl.body(),
        reader::MethodDefOrRef::MethodDef(method) if method.name() == "Ordinary"
    ));
    assert!(matches!(
        method_impl.declaration(),
        reader::MethodDefOrRef::MemberRef(member)
            if member.name() == "Ordinary"
                && member.parent().namespace() == namespace
                && member.parent().name() == "IRuntimeClass"
    ));
...
#[test]
fn merge_preserves_midlrt_method_impls() {
    let input = std::path::Path::new("../../winrt/activation/metadata.winmd");
    let output =
        std::env::temp_dir().join("windows_metadata_merge_midlrt_method_impls.winmd");

    merge().input(input).output(&output).merge().unwrap();

    let input = reader::Index::read(input).unwrap();
    let output = reader::Index::read(&output).unwrap();
    let input_count: usize = input.types().map(|ty| ty.method_impls().count()).sum();
    let output_count: usize = output.types().map(|ty| ty.method_impls().count()).sum();
