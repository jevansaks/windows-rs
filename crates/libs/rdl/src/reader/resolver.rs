...
pub struct Resolver<'a, 'input> {
    ...
    pub generic_args: &'a [metadata::Type],
}

impl Resolver<'_, '_> {
    pub(super) fn mark_import_shadowed(&self, name: &str) {
        for import in &self.file.imports {
            if !import.glob && import.local.as_deref() == Some(name) {
                import.shadowed.set(true);
            }
        }
    }
    ...
    pub fn resolve_path(&self, ty: &syn::Path) -> Result<metadata::Type, Error> {
        ...
        let has_super = ty.segments.iter().any(|segment| segment.ident == "super");

        if path.len() == 1 {
            if let Some(number) = self.generics.iter().position(|generic| *generic == path[0]) {
                self.mark_import_shadowed(&path[0]);
                if let Some(ty) = self.generic_args.get(number) {
                    return Ok(ty.clone());
                }
            ...
            match path[0].as_str() {
                "bool" => return Ok(self.shadow_builtin(&path[0], metadata::Type::Bool)),
                "i8" => return Ok(self.shadow_builtin(&path[0], metadata::Type::I8)),
                "u8" => return Ok(self.shadow_builtin(&path[0], metadata::Type::U8)),
                "i16" => return Ok(self.shadow_builtin(&path[0], metadata::Type::I16)),
                "u16" => return Ok(self.shadow_builtin(&path[0], metadata::Type::U16)),
                "i32" => return Ok(self.shadow_builtin(&path[0], metadata::Type::I32)),
                "u32" => return Ok(self.shadow_builtin(&path[0], metadata::Type::U32)),
                "i64" => return Ok(self.shadow_builtin(&path[0], metadata::Type::I64)),
                "u64" => return Ok(self.shadow_builtin(&path[0], metadata::Type::U64)),
                "f32" => return Ok(self.shadow_builtin(&path[0], metadata::Type::F32)),
                "f64" => return Ok(self.shadow_builtin(&path[0], metadata::Type::F64)),
                "isize" => return Ok(self.shadow_builtin(&path[0], metadata::Type::ISize)),
                "usize" => return Ok(self.shadow_builtin(&path[0], metadata::Type::USize)),
                "void" => return Ok(self.shadow_builtin(&path[0], metadata::Type::Void)),
                "String" => return Ok(self.shadow_builtin(&path[0], metadata::Type::String)),
                "Object" => return Ok(self.shadow_builtin(&path[0], metadata::Type::Object)),
                "Char16" => return Ok(self.shadow_builtin(&path[0], metadata::Type::Char)),
                _ => {}
            }
        }
    ...
    fn shadow_builtin(&self, name: &str, ty: metadata::Type) -> metadata::Type {
        self.mark_import_shadowed(name);
        ty
    }
