...
const UNUSED_IMPORT_CODE: &str = "RDL0006";
const SHADOWED_IMPORT_CODE: &str = "RDL0007";
...
pub(super) fn validate_symbols(index: &Index) -> Vec<Error> {
...
pub(super) fn validate_resolved_symbols(
    index: &Index,
...
pub(super) fn validate_use_declarations(
    input: &[File],
    index: &Index,
    reference: &metadata::reader::Index,
) -> Vec<Error> {
...
pub(super) fn validate_import_warnings(input: &[File]) -> Vec<Error> {
    let mut diagnostics = vec![];
    ...
    for file in input {
        for import in &file.imports {
            ...
            let start = import.span.start();
            let end = import.span.end();
            let target = display_import(import);

            if import.shadowed.get() {
                if import.allow_shadowed {
                    continue;
                }
                diagnostics.push(
                    Error::new(
                        &format!("import `{target}` is shadowed"),
                        &file.source,
                        start.line,
                        start.column,
                    )
                    ...
                    .with_code(SHADOWED_IMPORT_CODE)
                    .with_primary_label(
                        Label::primary(&file.source, start.line, start.column)
                            .with_source_id(file.source_id)
                            .with_end(end.line, end.column)
                            .with_message("this import cannot win name resolution"),
                    )
                    .with_help("remove or rename the import, or add `#[allow(shadowed_imports)]`"),
                );
            } else if !import.allow_unused {
                diagnostics.push(
                    Error::new(
                        &format!("unused import `{target}`"),
                        &file.source,
                        start.line,
                        start.column,
                    )
                    ...
                    .with_help("remove the import or add `#[allow(unused_imports)]`"),
                );
            }
        }
    }
...
fn display_import(import: &Import) -> String {
    let path = import.path.join(".");
    if import.glob {
        return format!("{path}.*");
    }
    if let Some(local) = &import.local
        && import.path.last() != Some(local)
    {
        return format!("{path} as {local}");
    }
    path
}
...
fn validate_interface(file: &File, item: &Interface) -> Result<(), Error> {
    ...
    for member in &item.members {
        ...
        match member {
            ...
            InterfaceMember::Property(_) | InterfaceMember::Event(_) => {}
        }
    }
...
fn duplicate_error(
    ...
    .with_source_id(current_file.source_id)
    ...
    .with_label(
        ...
        .with_source_id(previous_file.source_id)
        .with_end(previous_end.line, previous_end.column),
    )
}
