...
Print the same metadata view for an existing winmd:
...
reference metadata in a separate lookup index. `--profile common`, `win32`, `winrt`, or `windows`
selects policy validation; common is the default.
...
both metadata and projected names. `riddle dump` uses the same renderer for existing metadata.

RDL diagnostics use exit code 1. Invalid command lines use exit code 2. `--format human` prints
labeled source lines, notes, and help. `--format short` emits one location line per diagnostic.
`--format json` emits one document containing a `diagnostics` array and an error/warning `summary`;
it never emits ANSI escapes. `--color auto`, `always`, or `never` controls human and short output,
with `auto` honoring `NO_COLOR`. Every failing invocation prints one final error/warning count.
Formatting reads and validates every input before updating any file, so one invalid input leaves
the whole set unchanged. Warning-only checks still succeed; unused and shadowed imports are shown
on standard error with the selected format.
