...
`windows-reactor` is still pre-release. Prefer the clearest design and implementation over
preserving compatibility with earlier pre-release APIs.

### How it's built

The hooks runtime, element tree, reconciler, and WinUI backend are hand-written. `tool_reactor`
generates the per-widget dispatch from `crates/tools/reactor/src/winui.toml` plus the WinUI
`.winmd` metadata:
...
Regenerate with `cargo run -p tool_reactor`, then verify with
...
The WinUI backend keeps stateful lifetime and topology rules in focused owners:

| Module | Responsibility |
| --- | --- |
| `native_children.rs` | Logical child order and corresponding WinUI collection updates |
| `resources.rs` | Local resources, theme bindings, refresh, and per-control cleanup |
| `events.rs` | Event callbacks, observers, revokers, and their cleanup |

Event dispatch remains in `winui/mod.rs` and `generated_attach_event.rs`; `EventState` owns the
registrations and callback lifetimes rather than all attachment logic. `properties.rs` contains a
stateless `apply` function for hand-written properties. Its `PropertyContext` borrows the control
map, event state, and window state only for properties that need those services.

`animation.rs` groups Composition helpers, and `reconciler/modifiers.rs` groups modifier
application and diffing. These modules improve navigation but do not own persistent state.
...
The owners below keep topology changes and cleanup accounting behind narrow operations.
...
their child output mounts. `MountedLogicalTree` owns logical ID allocation, component and wrapper
records, parent links, native projections, listener counts, subtree removal, context subscriber
queries, and appeared/disappeared dispatch.

Component state holds its `RenderCx`, previous object, previous mounted output, and context
subscriptions. A logical parent path crosses intervening native widgets, so a stateful child
remains reachable through a transparent or memoized wrapper. A mounted output slot remains stable
when its logical node temporarily produces no native control.

### Ownership consolidation

`MountedTree` owns native nodes, parent links, `ChildSlots`, header and pane outputs, custom
handles, and pre-unmount callbacks. `ChildSlots` keeps the native sequence together with an
optional logical output mirror. The mirror is absent for backend-managed collections that do not
participate in element reconciliation.

Child insertion, replacement, movement, removal, and keyed permutation use `MountedTree`
operations. Callers cannot borrow the logical child vector mutably or update parent links
directly. Templated rows use focused attach and detach operations when their mounted output
changes.

### Traversal and cleanup

Mounted traversal includes primary children, header and pane outputs, and realized templated rows.
Unmount gathers the native subtree, gathers logical roots that have no native projection, removes
logical subtrees through `MountedLogicalTree`, and destroys native nodes child-first. Context
invalidation walks native children and logical roots through focused owner APIs. Realized-row
lifecycle traversal includes native descendants and logical-only roots without revisiting projected
nodes. Appeared callbacks run parent-first; disappeared callbacks run child-first.

`reconciler/context.rs` coordinates context invalidation across mounted and logical owners.
`reconciler/child.rs`, `wrappers.rs`, and `templated.rs` contain reconciliation algorithms; they do
not own the maps that define logical identity or native child topology. Debug builds verify
projection coverage, logical parent validity, unique native ownership, matching parent links, and
agreement between logical and native child order.
...
Post-render and native theme/size callbacks hold `WeakRenderHost`. Dropping the final strong host
therefore unmounts the native root and runs root hook cleanups, releasing references, effects,
